            ...
            for (const phrase of plotIntroPhrases) {
                ...
                if (lowerTextChunk.startsWith(lowerPhrase)) {
                    ...
                    if (urlPart.startsWith('/static/plots/') && urlPart.toLowerCase().endsWith('.png')) {
                        ...
                        img.className = 'block max-w-full h-auto mt-2 mb-2 rounded shadow-md'; // Ensured h-auto and shadow-md
                        ...
                        link.className = 'inline-block focus:outline-none focus:ring-2 focus:ring-blue-300 rounded'; // Added focus styling to link
    ...
    if (clearChatButton) {
        clearChatButton.addEventListener('click', () => {
            if (messagesDiv) {
                ...
                localStorage.removeItem(CHAT_HISTORY_KEY); // Clear client-side history

                // Signal backend to clear server-side history for this session
                if (websocket && websocket.readyState === WebSocket.OPEN) {
                    websocket.send(JSON.stringify({ action: "clear_server_history" }));
                    console.log("Sent clear_server_history signal to backend.");
                } else {
                    console.warn("WebSocket not open. Cannot send clear_server_history signal.");
                }

                addMessageToDisplay('status', 'Chat cleared and history erased (client-side).'); // Update status
                currentAiMessageElement = null;
                currentAiTextContentDiv = null;
                console.log('Chat cleared and history erased by user (client-side). Server notified if connected.');
            } else {
